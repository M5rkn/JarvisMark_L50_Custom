...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    for _ in range(30):
        ...
        if _is_steam_running():
            ...
            if is_windows():
                # Steam drops steam:// protocol commands fired while it is still
                # booting, so wait until its main window is actually visible.
                for _ in range(20):
                    time.sleep(1)
                    if _get_steam_window_rect():
                        break
                time.sleep(3)
...
def _restart_steam(steam_path: Path) -> str:
    exe = _steam_exe(steam_path)
    if not exe.exists():
        return f"Steam not found at {exe}."

    if _is_steam_running():
        print("[GameUpdater] 🔁 Steam kapatılıyor...")
        try:
            if is_windows():
            ...
            return f"Could not stop Steam: {e}"
        for _ in range(15):
            time.sleep(1)
            if not _is_steam_running():
                break

    print("[GameUpdater] 🚀 Steam yeniden başlatılıyor...")
    if is_mac():
        subprocess.Popen(["open", "-a", "Steam"])
    else:
        subprocess.Popen([str(exe)])
    ...
    return "Steam did not start after restart."


def _close_steam(steam_path: Path) -> str:
    if not _is_steam_running():
    ...
    try:
        if is_windows():
            subprocess.run(["taskkill", "/IM", "steam.exe", "/F", "/T"],
                           capture_output=True, **_CNW)
            subprocess.run(["taskkill", "/IM", "steamwebhelper.exe", "/F", "/T"],
                           capture_output=True, **_CNW)
        else:
            subprocess.run(["pkill", "-x", "steam"], capture_output=True)
    except Exception as e:
        return f"Could not close Steam: {e}"

    for _ in range(10):
        time.sleep(1)
        if not _is_steam_running():
            return "Steam closed."
    return "Steam may still be closing."
...
def _resolve_steam_game(steam_path: Path, game_name: str = None,
                        app_id: str = None) -> tuple[dict | None, str | None, str | None]:
    """Return (game_dict, resolved_app_id, resolved_name). game_dict is from local library if found."""
    installed = _get_steam_games(steam_path)

    if app_id:
        app_id = str(app_id)
        game = next((g for g in installed if g["id"] == app_id), None)
        if game:
    ...
    name_lower = game_name.lower()
    matches = [g for g in installed if name_lower in g["name"].lower()]
    if len(matches) == 1:
        g = matches[0]
        return g, g["id"], g["name"]
    if len(matches) > 1:
        names = ", ".join(g["name"] for g in matches[:5])
        return {"_ambiguous": names}, None, game_name

    found_id, found_name = _search_steam_appid(game_name)
    return None, found_id, found_name or game_name

def _launch_steam_game(steam_path: Path, game_name: str = None,
                       app_id: str = None) -> str:
    if not _ensure_steam_running(steam_path):
        return "Could not start Steam."

    exe = _steam_exe(steam_path)
    game, resolved_id, resolved_name = _resolve_steam_game(
        steam_path, game_name=game_name, app_id=app_id
    )

    if isinstance(game, dict) and game.get("_ambiguous"):
        return f"Multiple games match '{game_name}': {game['_ambiguous']}. Please be more specific."

    if game:
        state = game["state"]
        name  = game["name"]
        if state == 1026:
            return f"'{name}' is still downloading. Wait for it to finish."
        if state in (6, 516):
            return f"'{name}' has a pending update. Say 'update {name}' first."
        resolved_id = game["id"]
        resolved_name = name

    if not resolved_id:
        label = game_name or app_id or "game"
        return f"Could not find '{label}' on Steam."

    label = resolved_name or game_name or resolved_id
    url   = f"steam://rungameid/{resolved_id}"
    last_err = None
    # Steam can silently drop a steam:// command if it is still booting, so
    # fire it a few times with a pause — the command is idempotent.
    for attempt in range(3):
        try:
            _launch_steam_url(exe, url)
            print(
                f"[GameUpdater] 🎮 Launching: {label} (AppID: {resolved_id}) "
    ...
    if last_err:
        return f"Launch failed: {last_err}"
...
def game_updater(parameters: dict, player=None, speak=None) -> str:
    ...
    if action == "close":
        if platform in ("steam", "both"):
            steam_path = _find_steam_path()
            if not steam_path:
                msg = "Steam: Not installed."
            else:
                msg = _close_steam(steam_path)
            if player:
    ...
    if action == "launch":
        if platform in ("epic", "both") and platform != "steam":
            results.append("Epic: Launch via Epic Games Launcher is not supported yet.")
        if platform in ("steam", "both"):
            steam_path = _find_steam_path()
            if not steam_path:
                results.append("Steam: Not installed.")
            elif not game_name and not app_id:
                results.append("Steam: Please specify a game name to launch.")
            else:
                msg = _launch_steam_game(steam_path, game_name=game_name, app_id=app_id)
                if player:
                    player.write_log(f"[GameUpdater] {msg[:100]}")
                if speak:
                    speak(msg)
