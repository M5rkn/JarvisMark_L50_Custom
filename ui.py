...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "J.A.R.V.I.S", parent=None):
        ...
        self._blink      = True
        self._blink_tick = 0
        self._particles: list[list[float]] = []   # [x, y, vx, vy, life, depth, size]
    ...
    def _step(self):
        ...
        # radial pulse (speech / listening)
        self._pulse += dt * (0.35 if self.speaking else 0.10)
        if self._pulse > 1.0:
            self._pulse = 0.0

        # waveform — smooth pseudo-amplitude tied to state
        amp = 0.5 if self.speaking else (0.18 if self.state == "LISTENING" else 0.05)
        if self.muted:
            amp = 0.0
    ...
    def paintEvent(self, _):
        ...
        # 1) background — near-black with a very subtle blue falloff
        ...
        # 3) large subtle geometric structures (depth layer)
        ...
        # 5) tick marks on the outermost ring
        ...
        # 6) orbit data nodes with center links
        for k in range(3):
        ...
        for i in range(6):
            ...
            a = int(glow * 0.9 * frc)
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(qcol(C.MUTED_C if self.muted else C.PRI, max(0, min(255, a)))))
        ...
        # 9) radial pulse (speech / listening)
        if self.speaking or self.state == "LISTENING":
            pr = fw * (0.16 + self._pulse * 0.42)
            p.setPen(QPen(qcol(C.PRI, int(110 * (1.0 - self._pulse))), 1.2))
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.drawEllipse(QRectF(cx - pr, cy - pr, pr * 2, pr * 2))

        # 10) particles (depth = size & alpha)
        for pt in self._particles:
        ...
        # 11) rotating data labels (very faint)
        ...
        # 12) circular waveform around the core
        wv_r = fw * 0.30
        n = len(self._wave)
        path = QPainterPath()
        for i in range(n + 1):
            ...
            if i == 0:
                ...
                path.lineTo(x, y)
        p.setPen(QPen(qcol(C.MUTED_C if self.muted else C.PRI, min(255, int(110 + glow))), 1.3))
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawPath(path)

        # 13) status label — small and subtle
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        lay.addSpacing(4)
        lay.addWidget(_lbl("YOUR NAME  (optional — used in responses)", 8,
...
class MainWindow(QMainWindow):
    ...
    def _build_header(self) -> QWidget:
        ...
        self._title_lbl.setFont(QFont("Segoe UI", 19, QFont.Weight.Light))
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._voice_lbl = QLabel("🔊  100%")
        self._voice_lbl.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self._voice_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        lay.addWidget(self._voice_lbl)

        self._voice_slider = QSlider(Qt.Orientation.Horizontal)
        self._voice_slider.setRange(0, 100)
        self._voice_slider.setValue(100)
        self._voice_slider.setFixedHeight(18)
        self._voice_slider.setCursor(Qt.CursorShape.PointingHandCursor)
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input.setPlaceholderText("Speak or enter a command…")
    ...
    def _on_file_selected(self, path: str):
        ...
        if cat == "image":
            self._file_hint.setText(f"{icon}  {p.name}  ·  {size}  ·  Analyzing…")
        else:
    ...
    def _apply_state(self, state: str):
        ...
        _state_lbl, _state_col = self.hud._status_label()
        if hasattr(self, "_status_lbl"):
            self._status_lbl.setText(f"●  {_state_lbl}")
            self._status_lbl.setStyleSheet(
                f"color: {_state_col}; background: transparent;"
            )
